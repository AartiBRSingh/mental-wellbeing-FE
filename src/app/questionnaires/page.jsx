...
const Page = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(`${baseURL}/get-questions?userType=${userType}`);
        setQuestions(response.data); // Set questions from API response
      } catch (error) {
  ...
  const handleAnswerChange = (questionId, answer) => {
    setAnswers((prevAnswers) => ({
      ...prevAnswers,
      [questionId]: answer, // Update answers with selected choice
    }));
  };

  // Handle submit action
  const handleSubmit = async () => {
    const authToken = localStorage.getItem("authToken"); // Get JWT token from localStorage
    ...
    try {
      ...
      // Make POST request to save answers
  ...
  <div className="max-w-3xl mx-auto">
    <div className="text-center mb-12">
      <h1 className="text-4xl font-bold text-gray-900 mb-2">Case Study Questionnaire</h1>
          ...
          >
            ...
            <div className="px-8 py-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <div className={`mb-3 w-12 h-12 rounded-full flex items-center justify-center
                    ${answers[question._id] === 'yes'
                      ? 'bg-green-500'
                  ...
                  >
                    <svg 
                      className={`w-6 h-6 transition-colors duration-200
                        ${answers[question._id] === 'yes' ? 'text-white' : 'text-gray-400'}`}
                      fill="none" 
                      viewBox="0 0 24 24" 
                      stroke="currentColor"
                    >
                ...
                >
                  ...
                  >
                    <svg 
                      className={`w-6 h-6 transition-colors duration-200
                        ${answers[question._id] === 'no' ? 'text-white' : 'text-gray-400'}`}
                      fill="none" 
                      viewBox="0 0 24 24" 
                      stroke="currentColor"
                    >
