...
const NewsPage = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(`${baseURL}/get-categories?type=news`);
  ...
  const fetchPosts = async () => {
    try {
      let url = `${baseURL}/posts?type=news&category=${selectedCategory}&page=${currentPage}`;
  ...
  const fetchRecentPosts = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/posts?type=news&page=1&limit=3`
      );
      ...
      <div className="container mx-auto px-4 py-8">
        <header className="text-center mb-12">
          <h1 className="text-4xl font-extrabold text-gray-800 mb-3">
            Our <span className="text-[#FF844C]">News</span>
          </h1>
